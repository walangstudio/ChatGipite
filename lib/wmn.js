...
// undefined = not loaded; null = load failed (cached so we don't re-read a
// missing file every call); object = loaded OK.
let index;
function loadIndex() {
...
export async function checkWmn(site, account, { timeoutMs = 7000 } = {}) {
  ...
  try {
    const res = await fetch(url, {
      ...
      headers: site.headers || { 'User-Agent': BROWSER_UA, Accept: 'text/html,application/json' },
    });
