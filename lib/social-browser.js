...
const RULES = {
  ...
  linkedin: {
    ...
    taken: /\/company\/|companyName|org-top-card/i,
  },
  youtube: {
    notFound: /This page isn.?t available|404\b|does not exist/i,
...
// targets: [{ key, platform, url }] → { [key]: { available, status } }
export async function resolveViaBrowser(targets, { timeoutMs = 15000 } = {}) {
  const chromium = await loadPlaywright();
  if (!chromium) {
    return Object.fromEntries(
      targets.map((t) => [t.key, { available: null, status: 'playwright not installed — run: npx playwright install chromium' }]),
    );
  }
    ...
    for (const { key, url } of targets) {
      ...
      try {
        ...
        if (status === 404 || nf) {
          ...
          out[key] = { available: null, status: 'login/anti-bot wall — verify manually (browser)' };
        }
      } catch (e) {
