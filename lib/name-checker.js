...
import { resolveViaBrowser } from './social-browser.js';
import { wmnSiteFor, checkWmn } from './wmn.js';

const require = createRequire(import.meta.url);

// Opt-in WhatsMyName-driven detection (maintained signatures, direct request).
const WMN_ENABLED = process.env.CHATGIPITE_WMN === '1';
// Headless-browser fallback for HEAD-blocked socials. Opt-in: heavy (Chromium)
// and still can't beat IG/X login walls — see social-browser.js.
const PLAYWRIGHT_ENABLED = process.env.CHATGIPITE_PLAYWRIGHT === '1';
...
// These serve HTTP 200 + a login/redirect page for ANY handle when logged out,
// so a 200 says nothing about availability — must defer to the browser
// fallback or report unknown (never a false "taken").
const HEAD_UNRELIABLE = new Set(['instagram', 'tiktok', 'x', 'facebook']);

async function checkSocialHandle(platform, url, key) {
  try {
    ...
    if (res.status === 200 && HEAD_UNRELIABLE.has(key)) {
      return { platform, handle: url, available: null, status: 'HEAD inconclusive (login-walled) — needs browser/manual' };
    }
...
async function resolveSocial(key, handle) {
  const { platform, url } = { platform: SOCIAL_CHECKS[key].platform, url: SOCIAL_CHECKS[key].url(handle) };
  if (WMN_ENABLED) {
    const site = wmnSiteFor(key);
    if (site) {
      const r = await checkWmn(site, handle);
      return { platform, handle: url, available: r.available, status: r.status };
    }
  }
  return checkSocialHandle(platform, url, key);
}
...
export async function checkAvailability(name, opts = {}) {
  ...
  const [domains, socialResults] = await Promise.all([
    ...
    Promise.all(socials.map((s) => resolveSocial(s, handle))),
  ]);

  if (PLAYWRIGHT_ENABLED) {
    const targets = socials
      .map((key, i) => ({ key, i }))
      .filter(({ i }) => socialResults[i].available === null)
      .map(({ key, i }) => ({ key, platform: SOCIAL_CHECKS[key].platform, url: socialResults[i].handle }));
    if (targets.length) {
      const resolved = await resolveViaBrowser(targets);
      socials.forEach((key, i) => {
        const r = resolved[key];
        if (r) socialResults[i] = { ...socialResults[i], available: r.available, status: r.status };
      });
    }
  }
