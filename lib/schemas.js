...
export const schemas = {
  ...
  biz_canvas: z.object({
    idea_slug: slug,
    canvas_type: z.enum(['bmc', 'lean', 'vpc', 'mission', 'ai-platform']).optional().default('bmc'),
  }),

  biz_lean_canvas: ideaSlug,
  biz_value_prop: ideaSlug,
  biz_mission_canvas: ideaSlug,
  biz_ai_canvas: ideaSlug,
  ...
  biz_perspectives: z.object({
    idea_slug: slug.optional(),
    product: z.string().min(3).max(LONG_STR_MAX).optional(),
    constraints: optLong,
  }).refine((d) => d.idea_slug || d.product, { message: 'provide idea_slug or product' }),

  biz_product_psych: z.object({
    idea_slug: slug.optional(),
    product: z.string().min(3).max(LONG_STR_MAX).optional(),
    mode: z.enum(['apply', 'audit', 'full']).optional().default('full'),
  }).refine((d) => d.idea_slug || d.product, { message: 'provide idea_slug or product' }),

  biz_marketing_psych: z.object({
    idea_slug: slug.optional(),
    product: z.string().min(3).max(LONG_STR_MAX).optional(),
    mode: z.enum(['apply', 'audit', 'full']).optional().default('full'),
  }).refine((d) => d.idea_slug || d.product, { message: 'provide idea_slug or product' }),

  biz_engagement: z.object({
    idea_slug: slug.optional(),
    product: z.string().min(3).max(LONG_STR_MAX).optional(),
    mode: z.enum(['audit', 'apply', 'full']).optional().default('full'),
  }).refine((d) => d.idea_slug || d.product, { message: 'provide idea_slug or product' }),
  ...
  biz_north_star: ideaSlug,
  biz_assumptions: ideaSlug,
  biz_prfaq: ideaSlug,
  biz_mvp: ideaSlug,

  biz_rice_score: z.object({
    idea,
    context: optLong,
  }),

  biz_incubate: z.object({
    idea,
    sector: optShort,
    constraints: optLong,
  }),

  biz_launch_sprint: z.object({
    idea,
    sector: optShort,
    constraints: optLong,
  }),
