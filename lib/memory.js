...
function requireDb() {
  if (!db) throw new Error('memory not initialized — call memory.init(dbPath) first');
  return db;
}

export function store(type, content, metadata = {}) {
  requireDb().prepare('INSERT INTO memory (type, content, metadata) VALUES (?, ?, ?)').run(type, content, JSON.stringify(metadata));
}
