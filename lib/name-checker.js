...
async function checkSocialHandle(platform, url, key) {
  try {
    ...
    if (res.status === 200 && HEAD_UNRELIABLE.has(key)) {
      return { platform, handle: url, available: null, status: 'HEAD inconclusive (login-walled) — verify manually' };
    }
...
async function resolveSocial(key, handle) {
  const platform = SOCIAL_CHECKS[key].platform;
  const url = SOCIAL_CHECKS[key].url(handle);
  if (WMN_ENABLED) {
...
export async function checkAvailability(name, opts = {}) {
  ...
  if (PLAYWRIGHT_ENABLED) {
    const targets = socials
      .map((key, i) => ({ key, url: socialResults[i].handle, unresolved: socialResults[i].available === null }))
