...
import { checkAvailability } from './name-checker.js';
...
async function runCanvas(idea_slug, type) {
  const meta = CANVAS_TYPES[type] || CANVAS_TYPES.bmc;
...
Generate a diverse set, varying the styles.`;
  ...
  if (generatedNames.length > 0) {
    const dot = (a) => (a ? '✅' : a === false ? '❌' : '⚠️');
    const pick = (socials, platform) => dot(socials.find((s) => s.platform === platform)?.available);
    ...
    const rows = checks.map((r) => {
      ...
      return `| **${r.name}** | ${dot(dom.available)} \`${dom.domain}\` | ${pick(r.socials, 'X/Twitter')} | ${pick(r.socials, 'Instagram')} | ${pick(r.socials, 'LinkedIn')} | ${pick(r.socials, 'GitHub')} |`;
    });
    availabilitySection = [
      '', '---', '',
      '## Availability Check',
...
export async function handleNameCheck(args) {
  ...
  const mark = (a) => (a ? '✅ Available' : a === false ? '❌ Taken' : '⚠️ Unknown');
  const lines = [
    `# Availability Check: **${result.name}**`,
    `**Handle variant:** \`${result.handle}\``,
    '',
    `## Domains`,
    ...result.domains.map((d) => `- **${d.domain}:** ${mark(d.available)} (${d.status})`),
    '',
...
export async function handleRiceScore(args) {
  const { idea, context } = args;
  const spec = loadAgentSpec('validator');
  const task = `Compute a RICE score for this business idea: "${idea}". Reach (people/period), Impact (3=massive,2=high,1=med,0.5=low,0.25=minimal), Confidence (% as 1.0/0.8/0.5), Effort (person-months). RICE = (Reach × Impact × Confidence) / Effort. Show each input with rationale, the score, and a prioritization verdict vs a typical alternative.`;
  const output = await dispatch('validator', spec, task, context);
