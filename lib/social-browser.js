...
// Process-wide gate: only one Chromium runs at a time, even if several
// checkAvailability calls hit the browser path concurrently. Lets callers keep
// their normal name-level concurrency without spawning parallel browsers.
...
export async function resolveViaBrowser(targets, { timeoutMs = 15000 } = {}) {
  ...
  const run = browserGate.then(() => driveBrowser(ruled, out, timeoutMs));
  browserGate = run.then(() => {}, () => {});
  await run;
  return out;
}

async function driveBrowser(ruled, out, timeoutMs) {
