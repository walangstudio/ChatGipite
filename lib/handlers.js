...
Generate a diverse set, varying the styles.`;
  ...
  if (generatedNames.length > 0) {
    ...
    // Each name fans out to many TLD/social requests; with the heavy Playwright
    // fallback on, run names one at a time so we never spawn parallel Chromiums.
    const nameLimit = process.env.CHATGIPITE_PLAYWRIGHT === '1' ? 1 : 3;
    const checks = await mapLimit(generatedNames, nameLimit, (n) => checkAvailability(n));
